...
<tr>
  ...
  <th style={{position:'sticky',top:0,background:'#fff',zIndex:10}}>Spiel</th>
...
</thead><tbody>{matches.map(m=><tr key={m.id}><td>{m.match_number}</td><td>{new Date(m.kickoff_time).toLocaleString('de-DE')}</td><td>{m.team_a} – {m.team_b}</td>{players.map(p=><td key={p.id}>{tipFor(p.id,m.id)}</td>)}</tr>)}</tbody></table></div></div>; }
