...
.from(`tips`)
.select(`*`)
.range(0, 5000); setMatches(matchData||[]); setPlayers(playerData||[]); setTips(tipData||[]);} function tipFor(playerId:string,matchId:string){ const t=tips.find(x=>x.player_id===playerId&&x.match_id===matchId); return t?`${t.tip_goals_a}:${t.tip_goals_b}`:'';} return <div><h1>Alle Tipps</h1><div style={{ maxHeight: '75vh', overflow: 'auto' }}>
  <table><thead>
<tr>
...
</thead><tbody>{matches.map(m=><tr key={m.id}><td>{m.match_number}</td>
